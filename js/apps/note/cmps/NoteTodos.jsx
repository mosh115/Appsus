...
export class NoteTodos extends React.Component {
    ...
    todoSumbit = (ev) => {
        ...
        if (ev.keyCode === 13) {
            let { note } = this.state
            let todos = note.todos;
            const todo = { txt: ev.target.value, isDone: false }
            todos.push(todo)
            note.todos = todos
            this.setState({ note })
            ev.target.value = ''
